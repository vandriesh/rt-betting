...
export const EventItem = React.memo(({ event }: EventItemProps) => {
    ...
    return (
        <div className="grid grid-cols-[1fr,repeat(3,180px),80px] gap-4 p-4 items-center" data-testid="event-item">
            <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                    ...
                    {event.suspended && (
                        <span className="text-sm text-red-500" data-testid="suspended-indicator">
                            (Suspended)
                        </span>
                    )}
                </div>
            ...
            {mainMarket.selections.map((selection) => (
                <div key={selection.id} className="flex items-center justify-center space-x-2">
                    ...
                    <span className="text-xl font-bold w-16 text-center" data-testid="selection-price">
            ...
            >
                {event.suspended ? <Lock className="w-5 h-5" /> : <Unlock className="w-5 h-5" />}
            </button>
        </div>
    );
});
