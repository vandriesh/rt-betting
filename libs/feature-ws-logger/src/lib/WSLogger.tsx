...
export const WSLogger: React.FC<WSLoggerProps> = ({ title, maxMessages = 50 }) => {
    ...
    const addMessage = React.useCallback(
        (direction: 'in' | 'out', event: string, data: any) => {
            setMessages((prev) => {
                const newMessages = [
                    {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50 w-96 bg-gray-900 text-white rounded-lg shadow-lg">
            ...
            {/* Log Content */}
            {isExpanded && (
                <div ref={logContainerRef} className="h-96 overflow-y-auto border-t border-gray-700 text-sm font-mono">
                    {messages.map((msg) => (
                        <div key={msg.id} className="p-2 border-b border-gray-800 hover:bg-gray-800">
