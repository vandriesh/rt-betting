...
export const ChannelList: React.FC<ChannelListProps> = ({ eventChannels, marketChannels }) => {
    return (
                        ...
                        {eventChannels.map(({ channel, sources }) => {
                            ...
                            return (
                                <div key={channel} className="bg-white rounded-md shadow-sm border p-2">
                                    <div className="text-xs font-medium text-gray-900">Event #{id}</div>
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {marketChannels.map(({ channel, sources }) => {
                            ...
                            return (
                                <div key={channel} className="bg-white rounded-md shadow-sm border p-2">
                                    <div className="text-xs font-medium text-gray-900">Market #{id}</div>
                                    <div className="mt-1.5 flex flex-wrap gap-1.5">
                                        {sources.map((source) => (
                                            <SourceBadge key={source} source={source} />
                                        ))}
